...
void loop()
{
  if (Serial2.available())
  {
    ...
    if (response.startsWith("+RCV="))
    {
      ...
      // Cắt chuỗi theo định dạng chính xác
      String sender = response.substring(5, idx1);         // Địa chỉ gửi
      String length = response.substring(idx1 + 1, idx2);  // Độ dài
      String message = response.substring(idx2 + 1, idx3); // Nội dung
      String rssi = response.substring(idx3 + 1, idx4);    // RSSI
      String snr = response.substring(idx4 + 1);           // SNR
      ...
      u8g2.clearBuffer();
      u8g2.drawStr(2, 12, "Received:");
      u8g2.drawStr(2, 24, message.c_str());
